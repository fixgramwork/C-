...
public:
   ...
   // 객체가 메모리에서 제거될때 호출되어지는 함수가 있습니다.
   // 소멸자라고 합니다.
   // 소멸자를 만들지 않으면 컴파일러가 자동으로 생성해줍니다.
   // 소멸자에서는 객체 소멸될 때 해야하는 후단작업을 소멸자에서 합니다.
   // 리소스 정리등.
   ~SoccerSon() { // 소멸자
      cout << _name << "객체의 소멸자 호출" << endl;
...
// 생성자의 생성 규칙이 있습니다.
// 1. 생성자가 없으면 기본생성자를 자동으로 만들어 준다.
// 2. 인자를 받는 생성자가 하나라도 있으면 기본생성자를 자동으로 만들어주지 않는다.
